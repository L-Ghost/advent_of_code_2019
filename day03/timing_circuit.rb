class TimingCircuit
  ...
  attr_reader :wires_paths, :first_wire_horizontal_coordinates, :first_wire_vertical_coordinates, :second_wire_horizontal_coordinates, :second_wire_vertical_coordinates, :first_wire_steps, :second_wire_steps, :pointer, :step_counter
  ...
  def mark_step(wire)
    ...
    if wire == FIRST_WIRE
      first_wire_steps[coordinates] = step_counter unless first_wire_steps.include?(coordinates)
    else
      second_wire_steps[coordinates] = step_counter unless second_wire_steps.include?(coordinates)
    end
  end
  ...
  def lowest_steps 
    crossing_points.map do |coordinates|
      first_wire_steps[coordinates] + second_wire_steps[coordinates]
    end.min
  end
